	...
	for i in range(num_ag):
		if(i < num_riskT and i%2==0):
			if(c < num_comm):   
				...
				c += 1
			else: player = Agent(i+1, deepcopy(layout), exits, HEALTH_POINTS, 1, False)
			...
			if(c < num_comm):
				...
				c += 1
			else: player = Agent(i+1, deepcopy(layout), exits, HEALTH_POINTS, 0, False)
		elif(i >= num_riskT and (i%2!=0 or num_comm == 0)):
...
if __name__ == "__main__":
    ...
    res = 0
    for i in range(numIt):
        resa = mainCycle(None, com, not alar, numAg, 50, 50, rang, vol)
        res += resa
    final = (res/numIt)*100
    ...
    plt.ylim([0, 105])
    x = np.arange(2)
    plt.bar(x, height=[finalB, final])
    plt.xticks(x, ('Baseline With Alarm', 'Baseline Without Alarm'))
    ...
    plt.ylim([0, 105])
    plt.ylabel("Saved Evacuees (%)")
    plt.xlabel("Risk-Prone Evacuees (%)")
    plt.plot(x,y)
